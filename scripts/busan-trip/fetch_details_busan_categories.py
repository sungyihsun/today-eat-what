# -*- coding: utf-8 -*-
"""
One-off: fetch full Place Details for the final picked list of category-
expansion restaurants (see search_busan_categories.py). Same technique as
fetch_details_busan.py (NoRedirect photo-fetch pattern, Sunday=0->Monday=0
hours conversion).

FINAL_LIST_PATH: JSON written by the workflow's selection step, shaped like
{"燒烤": ["exact candidate name", ...], "海鮮": [...], ...}

Usage (CI): GOOGLE_PLACES_API_KEY=... python3 scripts/busan-trip/fetch_details_busan_categories.py
"""
import json, os, urllib.request, urllib.error, time

CANDIDATES_PATH = os.environ.get("BUSAN_CAT_CANDIDATES_PATH", "candidate-results/busan-cat-candidates.json")
FINAL_LIST_PATH = os.environ.get("BUSAN_CAT_FINAL_LIST_PATH", "candidate-results/busan-cat-final-list.json")
OUT_PATH = os.environ.get("BUSAN_CAT_DETAILS_PATH", "candidate-results/busan-cat-details.json")
...
final_names_by_cat = json.load(open(FINAL_LIST_PATH))
wanted = set()
for names in final_names_by_cat.values():
    wanted |= set(names)
by_name = {}
